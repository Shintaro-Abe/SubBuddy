...
2026-07-25時点で、iPhone向けWeb UIと見直し全出力は実装・自動試験済みで実機確認待ちである。通知は端末内予約、APNs送信待ち、お知らせ、削除予定日程、安全通知コマンドまで実装した。メール通知と通知用メールアドレス保存は行わない。サーバー通常通知の配信時間制御と、業務処理から通知作成までの障害時再実行は未実装である。機能フラグは初期オフで、これらの実装後にiOS・APNs・Render Cron・削除上流との実環境結合を行う。現行仕様は`docs/`、作業状態は`.steering/20260723-notification-delivery/tasklist.md`、手動確認は`manuals/notification-delivery-setup-and-check.md`を正とし、重複する技術メモは作らない。
