...
> 現行性注記（2026-07-21）：コード上のデータフローと境界は維持されている。期待値修正とSimulator起動前検査修復後にMac・iPhone実機回帰を行い、起動・復帰同期と通信復旧再送まで合格した。7日連続と実日付境界は引き続き別の確認対象である。
...
- 日をまたぐ連続利用の配分と7日連続計測は実機確認が残る。7日分の合成API回帰は合格済みだが、実時間の連続計測を代替しない。
- Mac Simulatorでは期待値修正と署名付きテストホスト修復後の全件再実行が残る。
- iPhone実機では起動・フォアグラウンド復帰の自動同期と、通信失敗後の自動再送が残る。
