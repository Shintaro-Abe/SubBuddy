...
| # | タスク | 対応AC | 状態 | 完了条件 |
|---|---|---|---|---|
| T-1 | `AuthSession`、client種別、期限、ローテーション、失効のPrisma schemaとmigrationを追加 | AC-2, AC-3, AC-10, AC-13 | 未着手 | unique・index・cascadeを含むmigrationが空DBと既存合成DBで通る |
| T-2 | 環境別issuer/audience/署名鍵/Cookie/期限の設定schemaとcloud起動時検証を実装 | AC-2, AC-5, AC-12, AC-14 | 未着手 | cloud設定不足で起動・要求が失敗しlocal actorへ戻らない |
| T-3 | JWT access tokenとopaque refresh tokenの発行・検証・hash保存を実装 | AC-2, AC-11 | 未着手 | 15分期限、必須claim、平文非保存を単体試験で確認 |
| T-4 | transactionによるrefresh rotation、並行更新制御、再利用時family失効を実装 | AC-3, AC-13 | 未着手 | 同一tokenの並行更新は1件だけ成功し、再利用後は系列全体が拒否される |
