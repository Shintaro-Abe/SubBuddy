...
| T-7 | iOS Keychain保存、単一refresh、401後1回再試行、再認証状態を実装 | AC-6, AC-13, AC-16 | 完了 | refresh競合・無限再試行・失効後の固定ユーザー化がない |
...
| T-13 | Render testflight環境へmigrationし、iPhone Appleサインインから契約APIまで実機確認 | AC-1, AC-6, AC-7, AC-16 | 完了 | 実データを証跡へ持ち出さず、対象環境で一気通貫を確認 |
...
- 2026-07-16 T-13（進行中）: Renderの配備先が`main`だったため、現行認証を含む`auth-tenant-boundary`へ切り替えてiPhoneのAppleサインイン成功を確認した。WebではApple JavaScript SDKへ生のnonceを渡す一方、サーバーがiPhoneと同じSHA-256値を期待して401となる不一致を検出した。Webは生のnonce、iPhoneはSHA-256値を照合するよう修正し、Web側も認証情報を含まない固定理由コードだけを記録する。
- 2026-07-16 T-13（進行中）: Web nonce修正後、対象テスト5件、Web全体27ファイル171件、型チェック、ESLintが合格した。Renderへ反映後、ブラウザのAppleサインインを再確認する。
- 2026-07-16 T-7/T-13: iOS単体テスト5件、iPhoneとWebのAppleサインイン、同じ内部ユーザーによる合成契約参照、16分後の自動更新、サインアウト、端末失効がすべて合格した。認証情報、Appleアカウント、端末ID、契約内容は証跡へ記録していない。
- 2026-07-16 T-7（再確認中）: 外部レビューで、サーバーのセッション更新成功後にKeychainの2項目目だけ保存失敗すると、古い更新トークンまたは途中保存値が残る可能性を確認した。途中失敗時に端末内セッションを全削除する修正と合成テストを追加する。
- 2026-07-16 T-7: Keychain部分保存防止の合成テストを含むiOS単体テスト6件がすべて合格した。途中失敗時の端末内セッション削除を確認し、T-7を完了へ戻した。
- 2026-07-16 T-7（再確認中）: Greptile最終レビューで、通信失敗時も端末内セッションは削除される一方、画面の`isSignedIn`が`true`のまま残る不整合を確認した。通信結果に関係なく端末側をサインアウト表示へ揃え、サーバー側失効を確認できなかった場合だけ結果文言を分ける。
- 2026-07-16 T-7: サインアウト表示修正後のiOS単体テスト6件がすべて合格した。端末内認証情報と画面状態を同じサインアウト状態へ揃え、T-7を完了へ戻した。
- 2026-07-16 T-7（再確認中）: Greptile最終レビューで、Appleセッション発行後に端末登録だけ失敗し、そのままAppleサインインを繰り返すと、到達不能なサーバーセッションが上限まで増える可能性を確認した。端末内トークン削除だけではサーバー側を失効できないため、新しいAppleセッション発行前に前回セッションを終了し、通信失敗時は新規発行へ進まない方式で修正する。
- 2026-07-16 T-7（再確認中）: 前回セッションを終了してから新しいAppleセッションを発行する修正を反映した。Linux環境にはXcodeがないため、このiOS差分を含む単体テスト6件の最終再実行はMacで行う。
- 2026-07-16 T-7/T-12: Apple障害の猶予期間終了を通常の期限切れと区別し、更新APIが503を返してiPhoneの保存済みセッションを維持するよう修正した。Web自動テスト28ファイル174件、型チェック、ESLint、本番ビルドが合格した。
- 2026-07-16 T-7: 端末登録失敗後の再試行修正を含むiOS単体テスト6件がMacのXcodeですべて合格した。T-7を完了とする。
