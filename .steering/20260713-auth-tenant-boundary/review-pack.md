...
| 受け入れ条件 | 設計要素 | タスク | 状態 |
|---|---|---|---|
| AC-1 | Apple subjectの共通内部user解決 | T-5, T-13 | 自動試験・Render実機ともに合格 |
| AC-2 | 15分access・1回限りrefresh | T-1, T-3, T-5 | 実装・自動試験済み |
| AC-3 | rotation・再利用検知・family失効 | T-1, T-4, T-12 | 実装・自動試験済み |
| AC-4 | Web Cookie・Origin・CSRF | T-6 | 実装・自動試験済み |
| AC-5 | 保持オフ・30日未使用・90日最長 | T-2, T-6 | 実装・自動試験済み |
| AC-6 | iPhone Keychain・単一更新・1回再試行 | T-7, T-13 | Xcode実機確認は合格済み。端末登録失敗後の再試行修正を含むiOS単体テストは最終再実行待ち |
| AC-7 | 全ユーザーAPIのactor必須化 | T-8, T-12, T-13 | 自動試験・Render実機ともに合格 |
| AC-8 | 別テナントIDの404拒否 | T-8, T-12 | 実装・自動試験済み |
| AC-9 | 利用量バッチ所有権と全件rollback | T-9, T-12 | 実装・自動試験済み |
| AC-10 | セッション10件・一覧・失効・全解除 | T-1, T-10, T-12 | 実装・自動試験済み |
| AC-11 | token hash保存・ログ非漏洩 | T-3, T-12 | 実装・検査済み |
| AC-12 | local限定固定user・cloud fail closed | T-2, T-8, T-11, T-12 | 実装・自動試験済み |
| AC-13 | logout・失効・削除後の拒否 | T-4, T-7, T-10, T-12 | 実装・自動試験済み |
| AC-14 | Apple障害時の既存session限定継続 | T-2, T-5, T-12 | 実装・自動試験済み |
| AC-15 | 2人以上の合成データ回帰 | T-12 | 自動試験済み |
| AC-16 | local回帰・Render実機確認 | T-7, T-11, T-13 | local・Render実機ともに合格 |
...
- 端末登録失敗後の再試行修正を含むiOS単体テスト6件の最終再実行。2026-07-16の実機確認後にiOSコードを変更したため、MacのXcodeで再確認する。
- Render上のWebとiPhoneによる共通ユーザー、認証済み契約API、16分後の自動更新、サインアウト、端末失効は確認済み。別ユーザーへの越境拒否は合成2ユーザーの自動試験で確認済み。
...
- [ ] 実装完了を承認（AC-1、AC-6、AC-7、AC-16の実環境確認後）
