...
| 受け入れ条件 | 設計要素 | タスク | 状態 |
|---|---|---|---|
| AC-1 | Apple subjectの共通内部user解決 | T-5, T-13 | 自動試験済み・実環境待ち |
...
| AC-6 | iPhone Keychain・単一更新・1回再試行 | T-7, T-13 | 実装済み・Xcode実機待ち |
| AC-7 | 全ユーザーAPIのactor必須化 | T-8, T-12, T-13 | 自動試験済み・実環境待ち |
...
| AC-16 | local回帰・Render実機確認 | T-7, T-11, T-13 | local合格・Render実機待ち |
...
- AC-1: Render上のWebとiPhoneで、同じAppleアカウントが同じ内部ユーザーになること。
- AC-6: Xcodeビルド後、iPhoneでKeychain保存、単一更新、401後1回再試行を確認すること。
- AC-7: Render上で認証済み契約APIを通し、別ユーザーのデータを取得・変更できないこと。
- AC-16: Render TestFlight環境でAppleサインインから契約APIまでを通し、結果をテスト監査台帳へ記録すること。
