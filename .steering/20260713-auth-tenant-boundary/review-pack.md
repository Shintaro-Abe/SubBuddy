...
| AC-6 | iPhone Keychain・単一更新・1回再試行 | T-7, T-13 | 自動試験・Xcode実機ともに合格 |
...
- なし。端末登録失敗後の再試行修正を含むiOS単体テスト6件はMacのXcodeで合格した。Render上のWebとiPhoneによる共通ユーザー、認証済み契約API、16分後の自動更新、サインアウト、端末失効は確認済み。別ユーザーへの越境拒否は合成2ユーザーの自動試験で確認済み。
