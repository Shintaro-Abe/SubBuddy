...
| 契約 | 一覧 / 追加 / 詳細 / 編集 / 削除確認 | CRUD、検索、絞り込み、不足情報 | AC-4, AC-5 |
...
| 削除確認 | 対象、影響、取消不能範囲を説明する | キャンセル、再認証して削除 |
...
- 後方互換 / マイグレーション: DBマイグレーションなし。既存Keychainトークンと計測選択を保持する。開発用API URL入力は利用者画面から除き、ビルド設定へ限定する。
