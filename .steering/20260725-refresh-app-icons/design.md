...
指定SVGを正本として`apps/ios/scripts/assets/app-icon-source.svg`へ配置する。Webファビコンでは正本SVGの全面背景だけを除去し、透過16・32・48px PNGを内包する`apps/web/src/app/favicon.ico`を生成する。iOSでは背景を維持したまま、再生成用1024px PNGと既存Asset Catalogの18サイズを生成する。
...
| 対象                                               | 変更内容                            | 対応AC           |
| -------------------------------------------------- | ----------------------------------- | ---------------- |
| `apps/web/src/app/favicon.ico`                     | 背景透過の16・32・48px内包ICOへ置換 | AC-1, AC-4       |
