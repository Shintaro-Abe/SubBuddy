...
- [x] **5.4** 未承認時は書かない仕組み（dry-run 既定・apply は明示フラグ＋確認ゲート）
  - 完了条件：承認なしに書き込まない（AC-2b）（✅ sync は既定 dry-run、apply は `--apply` 必須。承認は `/wbs-sync` の AskUserQuestion で担保）

## フェーズ 6：確認ゲート（Claude オーケストレーション）（WBS 6）

- [x] **6.1** 同期オーケストレータ（Claude Code の command）を定義
  - 完了条件：`--dry-run`→差分提示→**AskUserQuestion で承認**→承認時のみ `--apply` の流れが動く（✅ `.claude/commands/wbs-sync.md`）
- [x] **6.2** 差分の提示フォーマット（追加/更新/アーカイブを人が判断できる粒度）
  - 完了条件：承認前に変更内容が一覧で分かる（US-6 / FR-8 / AC-4）（✅ sync が件数＋ID＋タスク名＋変更フィールドを出力）

## フェーズ 7：自動トリガ（フック）（WBS 7）

- [x] **7.1** `wbs/scripts/detect-bolt-complete.mjs`：tasklist のチェックボックス解析で「全完了」を判定
  - 完了条件：全完了 / 一部未完 / タスクゼロ を正しく判定（✅ 5 シナリオで検証。保留 `[-]` は完了をブロックしない）
- [x] **7.2** 重複提案の抑止（最後に提案した状態のハッシュ等）
  - 完了条件：同一完了状態で繰り返し提案しない（✅ `wbs/.sync-state.json` にハッシュ記録・gitignore 済み）
- [x] **7.3** `.claude/settings.json` に `PostToolUse` フックを追加（`Edit|Write` → スクリプトで `.steering/*/tasklist.md` を判定）
  - 完了条件：tasklist 完了編集後に検知が走り、Claude へ同期提案が伝わる（FR-4）（✅ additionalContext で提案）
- [x] **7.4** 手動起動の併用確認
  - 完了条件：明示指示（`/wbs-sync`）でも同じオーケストレータが起動する（✅ 同一コマンド）
