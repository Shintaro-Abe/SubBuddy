...
- [ ] Q-1: フォントを system fallback のままにするか、`next/font/local`＋自ホスト（ライセンス確認）でデザインを厳密化するか。→ 今回は Render 投入優先で fallback を採用。後続で判断。
- [ ] Q-2: 本作業分のコミット範囲。作業ツリーには前セッション由来の無関係な変更も混在。`pre-commit-secret-scan` 実行後、本ステアリング関連にスコープを絞るか。
...
- [ ] 未決事項ゼロを確認（Q-1・Q-2 が残：後追いのため事後確認）
- [x] トレーサビリティ表に漏れ・孤立なし
