...
- ビルド修正：`next/font/google` はビルド時に Google Fonts へ外部アクセスし、制限環境で失敗する。`layout.tsx` から除去し、`globals.css` のフォント変数を system fallback に変更。
...
| `src/app/layout.tsx` / `src/app/globals.css` | next/font 除去・system フォント fallback | AC-9 |
...
- 既存コード・機能への影響：`registerDeviceForAppleUser` のシグネチャに `clientDeviceId` 追加（呼び出し側・テストを更新済み）。フォント変更で表示フォントが system 依存に変わる。
...
- 前提4：フォントは Render ビルドを外部取得に依存させないことを優先。デザイン厳密化は後続で自ホスト化を検討。
