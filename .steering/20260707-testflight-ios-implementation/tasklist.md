...
> 作成日：2026-07-07
> 現行性注記（2026-07-16）：後続の`.steering/20260713-auth-tenant-boundary/`で短期アクセストークンとローテーション更新トークンへ変更し、現行経路のXcode・Render実機確認まで完了した。iOSの利用者向け主製品UI、Archive・codesign、7日連続計測は`.steering/20260711-release-roadmap-rebaseline/`の後続タスクで扱う。
...
| T-13 | Greptileレビューのデータ整合性・PII・通信経路指摘を修正 | AC-7, AC-8, AC-12 | 完了 | App Groupのプロセス間競合を防ぎ、送信済みの同一版だけを削除し、端末名PIIを送らず、API URLをHTTPSに限定する。Mac/Xcodeの単体テスト6件が合格 |
...
- 2026-07-16: T-13のSharedStore競合・送信済み削除・HTTPS制約を含むiOS単体テスト6件がMacのXcodeですべて合格した。現行認証経路のWeb・iPhoneサインイン、16分後の更新、サインアウト、端末失効もRenderで確認済み。
