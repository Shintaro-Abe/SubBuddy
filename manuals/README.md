...
利用者向けアプリ名は`MUDASK`です。リポジトリ名、XcodeのTarget・Scheme・プロジェクト名、Bundle ID、App Group、環境変数、API URLなど、手順中の内部識別子は`SubBuddy`のまま使います。
...
| [notification-delivery-setup-and-check.md](./notification-delivery-setup-and-check.md) / [HTML](./notification-delivery-setup-and-check.html) | APNs、Render Cron、秘密値、5種類の合成実機確認、停止手順                                                                                | `.steering/20260723-notification-delivery/`                                                              |

## 現在まだ手順書がない機能

通知コードと確認手順は作成済みですが、機能フラグは初期オフです。サーバー通常通知の配信時間制御と、業務処理から通知作成までの障害時再実行を実装したうえで、MacでのiOS build・単体試験、iPhone実機APNs、Render Cron、削除予定との一気通貫が終わるまで利用可能表示へ切り替えません。
