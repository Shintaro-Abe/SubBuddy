...
- 現在の`NOTIFICATIONS_ENABLED`は初期値の`false`です。ステアリングのT-24（サーバー通常通知の配信時間制御）とT-25（業務処理から通知作成までの障害時再実行）が完了するまでは、ステップ6以降を実行しません。
...
> ✅ 確認：`20260723190000_add_notification_delivery`が適用済みと表示されます。
...
T-24とT-25を含むコード側の完成ゲート、資格情報、migration、Cron、iPhone署名を確認した後に、実機試験の間だけ利用可能状態へ切り替えます。
...
> ⚠ T-24またはT-25が未完了なら有効化しません。Web ServiceとCronの片方だけを有効にしません。設定不足時に500エラーになる場合はフラグを`false`へ戻し、秘密値のキー名だけを確認します。
...
5. 削除予定の日程計算、通知作成、取消は実装済みです。削除専用コード・無活動削除・TestFlight終了削除を発生させる上流処理が接続された後に、合成アカウントで90・30・7日前、申請直後・24時間前、終了時の各経路を確認します。上流処理がない間は未確認として残し、アプリ内とAPNsを合格扱いにしません。
6. 安全通知は最初にdry-runだけを行います。

```bash
cd /workspaces/SubBuddy/apps/web && npm run notifications:safety -- --incident-id synthetic-notification-check
```

7. 表示された対象件数と経路を確認した後、検証環境だけでapplyします。

```bash
cd /workspaces/SubBuddy/apps/web && npm run notifications:safety -- --incident-id synthetic-notification-check --apply
```

8. 同じ事故IDで再実行して二重作成されないことを確認します。
9. 通知をタップし、更新、同期、端末とセッション、お知らせの対応画面へ移動することを確認します。
