...
- **iPhone は利用量センサー**：Screen Time（DeviceActivity）の集計値のみを Mac へ同期する。
- **AI もクラウドも使わずに MVP を成立させる**：判断はルールベース。
...
| リスク | 影響 | 対応 |
|---|---|---|
| Family Controls entitlement 取得不可 | 利用量自動取得（MVP 中核）が成立しない | 先行 Spike（要求 10.3）。不成立時のみ代替計測方式を再評価 |
