...
- **デプロイ前処理**：Prisma migrationの後に、利用者データへ触れない冪等な専用処理で`service_catalog`、プラン、代替関係を同期する。利用者・契約を合成データへ入れ替える開発用seedはクラウド環境で実行しない。
...
- iPhoneは更新トークンとセッションIDを`ThisDeviceOnly`のKeychainへ保存し、アクセストークンはメモリだけに置く。同じAPI base URLでは`APIClient`を共有し、同時401時の更新処理を1つにまとめ、元の操作を各1回だけ再試行する。
- 目標仕様ではWeb有効セッションを最大10件、登録iPhoneを最大5台として別枠で扱い、一覧、個別失効、現在のサインアウト、全サインアウトを提供する。セッションに紐づく端末を失効した場合はデバイス同期トークンも失効する。
- 2026-07-25時点の現行コードは、Webセッション作成時だけ`clientType = web`の有効セッションを数え、iOSセッションをWebの10件上限へ含めない。セッションAPI、iPhone管理画面、Web設定の一覧・個別失効UIを実装している。登録iPhone5台の上限はWebセッション数とは分離し、デバイス登録境界で扱う。
...
- 2026-07-25時点のコードは、端末のIANAタイムゾーン登録、通常通知の現地時刻9〜20時制御、通知作成待ちのリース・再試行、APNs送信待ち、保持期限まで実装済みである。新規サインインでは認証セッションと通知作成待ちを同じDBトランザクションで保存し、通知イベント・お知らせ・配信対象へ冪等に展開する。外部APNs・Render Cron・iPhone実機の確認が終わるまで機能フラグを有効にしない。
