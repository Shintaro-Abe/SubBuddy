...
現行コードは利用実績、単価、`matchedPatterns`内の根拠・注意点を保存し、WebとiPhoneで表示する。利用日・利用分・1利用日単価を固定値で保存していた問題は修正済みである。一方、代替プラン・代替サービス・各節約額の専用保存項目、4つの確認優先度への表示移行、90日鮮度ルール、重大情報エラー時の表示停止は未完了である。
