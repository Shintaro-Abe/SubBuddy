...
2026-07-25時点の現行コードは、WebとiOSの有効セッションを合算して10件に制限している。セッション一覧・個別失効APIとiPhone画面はあるが、Web設定画面には一覧・個別失効UIがない。これは目標仕様との差分であり、複数ブラウザ利用を仕様として禁止するものではない。
...
現行実装には、利用者向け名称をMUDASKへ変更したWeb/iPhone画面、Webの主要MVP画面・API、使い方、設定、ログアウト、Apple認証セッション、テナント境界に加え、iPhoneの利用者向け初回導線、ホーム・契約・見直しの3タブ、支出、計測設定、同期、使い方、画面説明、端末・セッション、Apple再認証付き完全退会のSwiftUIコードがある。Webは320px以上で下部ナビと「その他」を使うコンパクトシェルを実装済みで、支出の内訳はモバイルで縦並び、PCで横棒表示になる回帰修正を確認済みである。計測設定は対応付け後の自動開始と、対象変更・解除時の旧計測データ削除・失敗後再試行へ変更した。開発用のAPI URL・内部契約ID入力は利用者画面から除去した。iPhoneのAppleログインは共有`APIClient`による更新競合修正後に利用者確認済みである。一方、Web・iOSセッションを合算する上限とWebのセッション管理UI不足は未解消である。
