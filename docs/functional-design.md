...
> 最終更新：2026-07-23（iPhone向けWeb、見直し、通知の実装境界を反映）
...
次表は外部TestFlight開始時の**目標仕様**であり、2026-07-23時点の完成状況ではない。
...
現行実装には、Webの主要MVP画面・API、使い方、設定、ログアウト、Apple認証セッション、テナント境界に加え、iPhoneの利用者向け初回導線、ホーム・契約・見直しの3タブ、支出、計測設定、同期、使い方、画面説明、端末・セッション、Apple再認証付き完全退会のSwiftUIコードがある。Webは320px以上で下部ナビと「その他」を使うコンパクトシェルを実装済みである。計測設定は対応付け後の自動開始と、対象変更・解除時の旧計測データ削除・失敗後再試行へ変更した。開発用のAPI URL・内部契約ID入力は利用者画面から除去した。
...
2026-07-23時点では、iPhone設定に「更新日の事前通知」「同期失敗の通知」のトグルと未接続の説明文があるが、画面全体を無効化している。通知許可要求、端末内予約、APNs、通知履歴、メール等の代替経路は未実装であり、通知は配信されない。
