...
```text
60日以上未使用                                  → strong_cancel_candidate
30日以上未使用 かつ 月額1,000円以上              → consider_cancel
同カテゴリに複数サービス かつ 片方が低利用       → 低利用側を consider_cancel
利用は少ないが importance が高い                → review
容量が余っている iCloud+                         → consider_downgrade
上記いずれにも該当せず十分利用                   → keep
```

- ルールのしきい値（未使用日数・金額・重複判定）は**設定値として外出し**し、調整可能にする（`architecture.md` 参照）。
- `reason` は MVP では上記ルールに対応する**定型文**を生成。フェーズ2 で AI による自然文生成に置換可能。
- 重複判定は `category` と `normalized_name` をもとに同カテゴリ複数契約を検出する。
...
- 認証は MVP ではローカル簡易認証（`architecture.md` で規定）。
