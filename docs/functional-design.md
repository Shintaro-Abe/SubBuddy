...
> 最終更新：2026-07-25（名称変更、認証セッション、通知、直近の画面修正を反映）
...
flowchart TB
  ...
  User -->|閲覧・登録| Web
  User -->|閲覧・登録・見直し| IOSApp
  Web --> API
  IOSApp -->|認証・契約・支出・見直し| API
  API --> DB
  API --> Scoring
  Scoring --> DB
```
...
| F-14 | 初回利用後の使い方ガイド       | 小規模検証版 | 棚卸し、支出・更新日、見直し、任意の利用状況設定の進捗共有と次操作の案内                                                                                             |
...
- **F-14 初回利用後の使い方ガイド**：iPhoneの既存初回導線を残し、完了後は既存ホームカードで次の1項目だけを示す。Webは契約0件時の短い初回案内と4項目の進み具合を示す。進捗は実操作から自動判定し、契約内容を含まない最小状態だけを利用者単位で保存する。Screen Timeは任意であり、見送りでも案内を完了できる。
...
erDiagram
  ...
  user_guidance_progress {
    ...
    datetime inventory_completed_at
    datetime spending_viewed_at
    datetime review_viewed_at
    string measurement_choice
  ...
  subscriptions {
    ...
    int plan_capacity_gb
    int used_capacity_gb
    datetime capacity_checked_at
...
| notes                | メモ                                                                                                       |
| plan_capacity_gb     | 契約中プランの容量（GB）。容量型契約だけ任意で保持                                                         |
| used_capacity_gb     | 確認した使用容量（GB）。容量型契約だけ任意で保持                                                           |
| capacity_checked_at  | 使用容量を確認した日時。容量情報の鮮度判定に使用                                                           |
...
Webの保持ログインがオフならブラウザsession Cookieかつサーバー側24時間、オンなら30日未使用または発行から90日の早い方で失効する。iPhoneも30日未使用または90日最長とする。目標仕様ではWeb有効セッションを1ユーザー10件までとし、登録iPhone5台とは別枠で扱う。利用者向け一覧にはクライアント種別、任意端末名、作成日時、最終利用日時だけを返し、IPアドレスや詳細User-Agentは保存しない。

2026-07-25時点の現行コードは、Webセッション作成時だけ有効なWebセッションを数え、iOSセッションをWebの10件上限へ含めない。セッション一覧・個別失効API、iPhone画面に加え、Web設定画面でも現在の接続を区別した一覧と、現在以外の接続を失効する操作を提供する。iPhone接続を失効した場合は、再ログインまでそのiPhoneの同期も停止する。

iPhoneは同じAPI base URLごとに`APIClient`を共有し、複数の通信が同時に401となった場合も更新処理を1つだけ実行する。更新成功後は元の通信を各1回だけ再試行する。

Appleサインイン障害中は環境設定の障害開始時刻を基準に新規セッション交換を停止する。障害前から存在するトークン系列だけを、通常のidle/absolute期限内かつ障害開始から最大72時間まで利用できる。

#### user_guidance_progress（初回利用後の案内進捗）

利用者ごとに1行だけ持つ最小進捗。契約内容や閲覧履歴全体は複製せず、棚卸し完了日時、支出・見直しの初回表示日時、利用状況を設定したか見送ったかだけを保存する。`user_id`は主キー兼外部キーで、アカウント削除時に連鎖削除する。同じ固定イベントの再送は成功扱いとし、iPhoneとWebで進捗を共有する。
...
利用者向けには`review_priority`、`review_unknowns`、`review_options`を保存する。`review_priority`は
`now` / `before_renewal` / `missing_information` / `low_urgency`の4値であり、内部`decision`を画面へ直接出さない。
解約、下位プラン、代替サービスの年間差額は別項目に保存し、選択肢には比較対象、月額、計算条件、情報源URL、確認日を含める。
`source_subscription_updated_at`が現在の契約更新時刻と一致しない旧結果は表示しない。
...
`confidence`（暫定/確定の確からしさ）で、**段階的な情報提供**（§8.6）の状態を表す。
...
`is_free_tier=true` のプランは比較対象から除外する。`verified_at`と`source_url`で料金の確認日と情報源を持つ。
公式確認から90日を超えた情報、情報源のない情報は節約額・確認優先度の根拠に使わない。配備やカタログ同期だけで確認日を更新せず、期限切れ料金へ係数を掛けた疑似価格も作らない。

#### service_alternatives（代替サービス）

サービス間の代替関係。`from_service_id` → `to_service_id` の関係で、P4（安い競合がある）の判定に使用する。
`relation` は `same_category` 等。`verified_at`と`source_url`があり、代替先プランも90日以内に確認済みの場合だけ見直し候補へ使う。
...
flowchart TB
  subgraph WebApp[Web（Next.js）]
    Pages[画面: ダッシュボード/契約/支出/見直し/更新/使い方/設定]
  ...
  subgraph APILayer[API 層]
    ...
    GuideAPI[guidance-progress API]
  end
...
- **案内進捗（Guidance API）**：契約の存在と固定イベントから4段階を解決し、iPhoneには次の1項目、Webには全体状態を返す。画面説明の既読状態はクライアントごとに端末内保存する。
...
- **トリガー**：iPhoneの`見直し`画面またはWebの明示的な再計算操作に加え、契約の登録・編集、利用量同期、容量更新、計測対象の変更・解除で対象契約だけを再計算する。
- **基本フロー**：入力（subscriptions・billing_events・usage）→ ルール適用 → 内部判定、確認優先度、根拠、不足情報、選択肢、差額 →
  `recommendation_snapshots` に履歴保存。
- **事後条件**：安全検査に合格した最新結果がiPhoneとWebに表示可能になる。再計算に失敗した場合は古い結果を表示せず、再計算案内を出す。
...
| reviewPriority      | 表示                                 | 意味                                                 |
| ------------------- | ------------------------------------ | ---------------------------------------------------- |
| now                 | 今確認したい                         | 更新、重複、料金差など、今確認する具体的な材料がある |
| before_renewal      | 更新前に確認したい                   | 更新日までに確認すべき材料がある                     |
| missing_information | 情報が不足している                   | 判断に必要な利用量、容量、料金情報などが足りない     |
| low_urgency         | 現時点では急いで確認する材料が少ない | 継続推奨ではなく、現時点の入力では急ぐ根拠が少ない   |

### 8.2 内部判定カテゴリ

`decision`は既存データと旧クライアントとの互換用に保持する。利用者向け画面には表示しない。
...
料金データには`verifiedAt`と`sourceUrl`を持たせる。90日を超えた情報や出典不明の情報は節約額・確認優先度の根拠に使わない。配備時に確認日を更新せず、古い料金への係数補正も行わない。
...
| 画面              | 内容                                                                                                                                 |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| ダッシュボード    | 月額/年額合計、今確認したい件数、更新間近の件数                                                                                      |
| サブスク一覧      | 登録済みサブスク、料金・更新日・確認優先度                                                                                           |
| サブスク詳細      | 詳細情報、単価、確認優先度、根拠、注意点、不足情報、選択肢、手続き導線                                                               |
...
│ サブスク一覧                          [+ 登録]  │
├───────────────────────────────────────────────┤
│ Amazon Music   ¥1,080/月  更新 6/12 [情報不足]   │
│ 日経電子版     ¥4,277/月  更新 6/20 [今確認]     │
│ iCloud+ 200GB  ¥400/月    更新 6/05 [今確認]     │
│ AIツールX      ¥3,000/月  利用量不明 [情報不足] │
│ 新規登録サービス ¥980/月  更新 6/30 [観測中 あと9日]│
└───────────────────────────────────────────────┘
```

> 登録直後の `観測中` でも、同カテゴリ重複・割高・更新間近の指摘は即時に表示する（§8.6）。
...
│ 1利用日あたり単価: ―（未使用）                  │
│ 確認優先度: 情報が不足している                   │
│ 分かっている事実: 過去30日の利用記録は0日        │
│ まだ分からないこと: 他の端末での利用状況          │
│ 選択肢: 必要性を確認 / 登録した手続きページを開く │
└───────────────────────────────────────────────┘
```
...
| GET      | `/api/guidance-progress`         | 本人の案内進捗、4段階の解決状態、次の操作を取得                                                                                |
| PATCH    | `/api/guidance-progress`         | 棚卸し完了、画面表示、利用状況の設定・見送りを固定イベントとして冪等更新                                                       |
| GET      | `/api/health`                    | Renderの起動確認。DB・Apple設定・secretの値は返さず、稼働状態と実行モードだけを返す                                            |
...
次表は外部TestFlight開始時の**目標仕様**であり、2026-07-25時点の完成状況ではない。
...
現行実装には、利用者向け名称をMUDASKへ変更したWeb/iPhone画面、Webの主要MVP画面・API、使い方、設定、ログアウト、Apple認証セッション、テナント境界に加え、iPhoneの利用者向け初回導線、ホーム・契約・見直しの3タブ、支出、計測設定、同期、使い方、画面説明、端末・セッション、Apple再認証付き完全退会のSwiftUIコードがある。Webは320px以上で下部ナビと「その他」を使うコンパクトシェルを実装済みで、支出の内訳はモバイルで縦並び、PCで横棒表示になる回帰修正を確認済みである。計測設定は対応付け後の自動開始と、対象変更・解除時の旧計測データ削除・失敗後再試行へ変更した。開発用のAPI URL・内部契約ID入力は利用者画面から除去した。iPhoneのAppleログインは共有`APIClient`による更新競合修正後に利用者確認済みである。Web有効セッションはiOSと別に10件までとし、Web設定から有効接続を確認・個別失効できる。App Iconとファビコンは専用正本から生成し、ワードマークとキャラクターは`assets/brand/`の共通正本からWebとiOS Asset Catalogへ同期する。キャラクターはサインイン・初回案内に限定し、データ中心画面には常設しない。

2026-07-17に新UIのSimulator buildとiOS単体テスト22件以上、実機API結合、標準・大画面、VoiceOver、最大Dynamic Type、200契約時の性能確認が合格した。2026-07-21に、案内状態とホームカード、Apple公式ボタンの高さ44pt・最大幅320pt、ライト・ダークで切り替わる通常操作色、自動同期をMac・iPhone実機で回帰確認した。同日、Webは320〜430px、横向き、200%相当、合成200契約とPC表示の自動回帰に合格したが、Safari・Chrome実機、VoiceOver、ソフトウェアキーボード、実機safe areaは未確認である。金額未入力保存、見直しの代替案・節約額全出力、暗号化オフライン編集、競合解決、アプリロック、データ出力、問い合わせ送信、通知配信は後続タスクが残り、外部TestFlight開始条件には未到達である。

### 14.2 画面構成

- iPhoneの主タブは「ホーム」「契約」「見直し」とし、設定はホーム右上から開く。
- ホームは年間支出、更新間近、棚卸し再開を示す。はじめ方が未完了の間だけ「次に確認すること」に`はじめ方 n/4`と次の1操作を表示し、初回導線の最初の見直し、または通常画面の見直し詳細1件で見直し確認を満たして4段階を完了した後は同カードを残さない。以後の見直し候補は主タブの「見直し」で確認する。
- 契約は一覧、検索、追加、編集、不足情報、計測設定、容量管理を扱う。
- 見直しは確認優先度、事実、対象期間、根拠、分からないこと、注意点、選択肢、公式導線を扱う。
- Webは契約0件なら登録を主操作にした短い初回案内、既存契約があれば4項目の進み具合を表示する。完了後もサイドバーの「使い方」から再表示できる。
- 利用者向け名称はiPhoneとWebで「契約」「支出」「見直し」「更新間近」にそろえる。既存URLとAPIの`recommendations`は内部互換のため維持する。
- 判断記録の3択と自動解約は初回版に含めない。

#### iPhone向けWeb

- 端末・ブラウザ名では分岐せず、1023px以下をコンパクトシェル、1024px以上を現行の左サイドバーとして同じHTMLを再配置する。
- コンパクトシェルの下部ナビは「ホーム・契約・見直し」の3項目とする。支出の内訳、更新間近、使い方、設定は全閲覧画面右上の「その他」と画面内の直接導線から開く。
- ログイン、契約追加・編集、削除確認では下部ナビを隠し、閲覧画面では詳細・補助画面を含めて表示する。
- ホームは今月の支出、未完了の次の操作、更新・見直し、詳しい概況の順とする。支出の月次推移はiPhoneで月・比較バー・金額の縦リスト、PCで棒グラフとする。
- 契約・見直し一覧は1列カード、詳細は概要・契約情報・見直し・利用状況の1列、フォームは1列・末尾全幅保存とする。名称と理由は一覧で最大2行、詳細で全文を表示する。
- `env(safe-area-inset-*)`、44px操作領域、入力文字16px以上、意味のあるDOM順序、フォーカス復帰、200%相当の文字拡大を前提とする。ページ全体の横スクロールで崩れを隠さない。

画面は、空、読込中、通信失敗、権限拒否、同期失敗、情報不足、保存中、削除確認、再認証要求を通常の状態として扱う。通信失敗で読込済み内容や入力下書きを消さず、再試行またはScreen Timeを使わない別経路を示す。

### 14.2.1 はじめ方と画面説明

はじめ方は、棚卸し、支出・更新日、見直し、任意の利用状況設定の4段階とする。契約の存在は契約データから導出し、棚卸し完了、支出表示、初回導線の最初の見直しまたは通常画面の見直し詳細表示、利用状況の設定または見送りだけを固定イベントとして保存する。同じイベントの再送は成功扱いにし、完了状態を巻き戻さない。

iPhoneは進捗APIが一時的に使えなくても通常機能を止めず、端末内に未送信イベントを保持して再送する。サインアウト時は別アカウントへ引き継がない。WebはScreen Time設定を行わず、必要な場合だけiPhoneで設定できると示す。

契約、支出、見直し、更新間近の初回表示では、画面の目的と表示対象を1〜2文の閉じられる画面内案内で示す。説明は操作を覆わず、「この画面について」から再表示できる。既読状態は画面構成が異なるiPhoneとWebで共有しない。
...
現行コードは利用実績、単価、`matchedPatterns`内の根拠・注意点に加え、代替候補、3種類の年間差額、不足情報、選択肢、4つの確認優先度を保存する。利用0日だけでは優先度を上げない。カタログ料金は出典付き・確認後90日以内だけを使い、配備日を確認日にしない。

保存時と読取時に見直し情報を検査する。所有者、契約更新時刻、登録金額との一致、情報鮮度、不足情報、優先度の根拠、節約額の計算条件を満たさない結果は本文を返さず、再計算案内だけを返す。契約登録・編集、利用量同期、容量更新後は対象契約だけを再計算し、失敗時も古い結果を表示しない。Webは合成データの自動試験まで合格し、iPhoneのSwiftUI実装はMac build・実機回帰待ちである。
