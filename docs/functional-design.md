...
> 最終更新：2026-07-20（初回利用後の使い方ガイドを反映）
...
- ホームは年間支出、更新間近、棚卸し再開を示す。はじめ方が未完了の間だけ「次に確認すること」に`はじめ方 n/4`と次の1操作を表示し、見直し詳細を1件確認して完了した後は同カードを残さない。以後の見直し候補は主タブの「見直し」で確認する。
...
はじめ方は、棚卸し、支出・更新日、見直し、任意の利用状況設定の4段階とする。契約の存在は契約データから導出し、棚卸し完了、支出表示、見直し詳細表示、利用状況の設定または見送りだけを固定イベントとして保存する。同じイベントの再送は成功扱いにし、完了状態を巻き戻さない。
