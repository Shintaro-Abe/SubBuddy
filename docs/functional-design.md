...
> 最終更新：2026-07-21（案内完了状態、Web設定、iPhone共通操作スタイルを反映）
...
次表は外部TestFlight開始時の**目標仕様**であり、2026-07-21時点の完成状況ではない。
...
現行実装には、Webの主要MVP画面・API、使い方、設定、ログアウト、Apple認証セッション、テナント境界に加え、iPhoneの利用者向け初回導線、ホーム・契約・見直しの3タブ、支出、計測設定、同期、使い方、画面説明、端末・セッション、Apple再認証付き完全退会のSwiftUIコードがある。計測設定は対応付け後の自動開始と、対象変更・解除時の旧計測データ削除・失敗後再試行へ変更した。開発用のAPI URL・内部契約ID入力は利用者画面から除去した。

2026-07-17に新UIのSimulator buildとiOS単体テスト22件以上、実機API結合、標準・大画面、VoiceOver、最大Dynamic Type、200契約時の性能確認が合格した。2026-07-21に、案内状態とホームカード、Apple公式ボタンの高さ44pt・最大幅320pt、ライト・ダークで切り替わる通常操作色、自動同期をMac・iPhone実機で回帰確認した。金額未入力保存、見直しの代替案・節約額全出力、暗号化オフライン編集、競合解決、アプリロック、データ出力、問い合わせ送信、通知配信は後続タスクが残り、外部TestFlight開始条件には未到達である。
