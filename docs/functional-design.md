...
| F-09 | iOS 計測対象選択 | MVP | FamilyActivityPickerで1契約につき1アプリを選択。状態を端末内で復元し、同じアプリの契約間重複を拒否 |
...
| 設定（iOS側） | 1契約につき1つの計測対象アプリを選択（FamilyActivityPicker）。停止と紐付け解除を分離 |
...
現行実装には、Webの主要MVP画面・API、Apple認証セッション、テナント境界に加え、iPhoneの利用者向け初回導線、ホーム・契約・見直しの3タブ、支出、計測設定、同期、設定、端末・セッション、Apple再認証付き完全退会のSwiftUIコードがある。開発用のAPI URL・内部契約ID入力は利用者画面から除去した。
