...
> 最終更新：2026-07-15（認証実装、iOS保存方式、実装状態を反映）
...
次表は外部TestFlight開始時の**目標仕様**であり、2026-07-15時点の完成状況ではない。
...
現行実装は、Webの主要MVP画面・API、Apple認証セッション、テナント境界、iPhoneの開発用認証・計測・同期UIまでである。iPhoneの契約・支出・見直し・設定・完全退会の利用者向けUI、オフライン編集、競合解決、アプリロック、出力は後続実装であり、外部TestFlight開始条件には未到達である。
