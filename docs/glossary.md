...
| 用語     | 定義                                                                                                                                                                            |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MUDASK   | Web版、iPhoneアプリ、Apple Developer上の利用者向けDescriptionなどに表示する製品名。利用者向けの文章ではこの名称を使う。                                                         |
| SubBuddy | リポジトリ名、ソース上のモジュール・Target・Scheme、Bundle ID、App Group、Cookie、環境変数、API URL、DB名などに残す内部プロジェクト名・識別子。互換性を壊す一括置換は行わない。 |
...
| スコアリング             | Scoring              | `scoring`                                    | 6つの検出パターン（P1〜P6）と該当なしのフォールバックでレコメンドを判定する処理。利用量はP1の入力のひとつであり中核ではない。AIを使わない（MVP）。                                                                                                                                                              |
...
| 知識ベース               | Knowledge Base       | —                                            | サービスカタログ・プラン・代替サービスの総称。安いプラン・代替候補の情報源。情報源付きで公式確認から90日以内の情報だけを節約額・確認優先度の根拠に使う。                                                                                                                                                        |
| 確認優先度               | Review Priority      | `review_priority`                            | 利用者が契約を確認する順番。「今確認したい」「更新前に確認したい」「情報が不足している」「現時点では急いで確認する材料が少ない」の4値。継続・変更・解約の結論ではない。                                                                                                                                         |
| 見直しの不足情報         | Review Unknown       | `review_unknowns`                            | 観測中、計測外、容量未確認、カタログ期限切れなど、SubBuddyだけでは判断できない事実。                                                                                                                                                                                                                            |
| 見直しの選択肢           | Review Option        | `review_options`                             | 継続、利用確認、重複確認、下位プラン、代替サービス、解約条件等、利用者が検討できる行動。比較金額を示す場合は計算条件・情報源・確認日を含む。                                                                                                                                                                    |
...
各判定はP1〜P6の検出パターンと、該当なしのフォールバックで導出される（§4a参照）。

> SubBuddy は**自動解約を行わない**。判定は提案までで、実行はユーザーに委ねる（恒久方針）。
> **`観測中`（`data_status = observing`）は判定値ではなく別軸の状態**。利用ベース判定が未確定の間の表示で、確定後に確認優先度へ反映する（`functional-design.md` §8.6）。

---

## 5a. パターンマッチング判定（P1〜P6 + 該当なし）
...
| 該当なし | 上記に該当なし                                           | 内部互換値`keep`   | `matchedPatterns`には保存しない                |
...
| 小規模検証版          | 30人募集、20人以上の有効参加者で判定し、50人を上限とするTestFlight検証版。一般公開前に、価値・信頼・運用可能性を確認する段階。                                                            |
...
| cloud-testflight mode | 小規模検証版を動かすクラウド実行モード。Renderシンガポール、マネージドPostgreSQL、Appleサインイン、認証セッション、デバイス同期トークンを使う。                                           |
...
| 認証セッション        | WebまたはiPhoneのログイン状態。Web有効セッション10件と登録iPhone5台を別枠で管理する。現行実装はWebセッション作成時だけ有効Webセッションを数え、iOSをWeb上限へ含めない。                   |
...
| 集計値               | iPhone が SubBuddy API へ送る、詳細ログを含まない要約値。現行Screen Time同期では契約ID、利用日、利用有無、利用バケット、概算時間範囲、取得源を扱う。                                                                                 |
...
| iPhone向けWeb    | Compact Web                | iPhoneのブラウザで利用するMUDASK Web。PC版と同じ機能・データを扱い、「ホーム・契約・見直し」の下部ナビを使う。ブラウザや端末モデルごとの別製品には分けない。                                                                   |
| PC向けWeb        | Desktop Web                | 幅の広いブラウザで利用するMUDASK Web。主要移動には左サイドバーを使う。                                                                                                                                                         |
| 主要画面         | Primary Screen             | 日常的に行き来する「ホーム・契約・見直し」の3画面。iPhone向けWebでは下部ナビから直接開く。                                                                                                                                     |
| 補助画面         | Secondary Screen           | 主要画面の判断や管理を補う「支出の内訳・更新間近・使い方・設定」。iPhone向けWebでは各画面の直接導線に加え、右上の「その他」から開く。                                                                                          |
...
| はじめ方         | Getting Started            | 棚卸し、支出・更新日、見直し、任意の利用状況設定の4段階を、実際の操作へ直接つないで案内する機能。iPhoneは未完了の次の1項目だけを表示し、4段階完了後はホームの案内カードを消す。Webは全体を表示する。                           |
...
| 設定（Web）      | Web Settings               | cloud modeでは現在のブラウザからのログアウト、有効セッション一覧、現在以外の接続の個別失効を提供する。local modeではログインを使わないため、ログアウトとセッション管理を表示せず説明だけを示す。                               |
| 通知設定         | Notification Settings      | 利用者単位の通知希望と、このiPhoneのOS許可・配信状態を分けて管理する画面。更新日前、同期失敗、新規サインイン、お知らせを確認できる。機能フラグ無効時は準備中と表示する。                                                       |
| お知らせ         | Notice                     | 新規サインイン、削除予定、安全性・長期障害だけをWebとiPhoneで確認するアプリ内履歴。更新日前と同期失敗は各状態画面で確認し、履歴へ保存しない。                                                                                  |
| 通知希望         | Notification Preference    | 通知種類ごとに利用者が受け取りたいかを示すクラウド同期設定。iPhoneのOS許可や端末トークンとは別に管理する。                                                                                                                     |
| 端末の配信状態   | Device Delivery State      | 特定iPhoneのOS許可、APNsトークン、このiPhoneで受け取る設定、端末内予約を合わせた状態。                                                                                                                                         |
| 通知作成待ち     | Notification Creation Task | 認証などの業務処理と同時にDBへ保存し、通知イベント・お知らせ・APNs配信対象の作成が失敗してもCronで再実行する最小記録。契約情報や端末トークンは含めない。                                                                       |
...
- **利用量同期と見直し再計算**：自動同期は日別集計をDBへ保存し、影響した契約だけを続けて再計算する。再計算に失敗した場合は古い見直し結果を表示せず、再計算を案内する。
...
| TestFlight小規模検証 | 30人募集、20人以上の有効参加者で判定し、50人を上限として価値、信頼、運用可能性を確認する限定リリース。 |
