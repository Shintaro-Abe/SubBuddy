...
| アクセストークン | 通常APIを呼ぶための15分有効な署名付きトークン。`userId`とセッションIDを含み、失効済みセッションでは使用できない。 |
| 更新トークン | アクセストークンを再発行する一度限りのランダム値。使用後はローテーションし、サーバーにはハッシュだけを保存する。 |
| トークン系列 | 1回のログインから更新で連なるセッション世代のまとまり。使用済み更新トークンの再利用を検知した場合、この単位で失効する。 |
| 認証セッション | WebまたはiPhoneのログイン状態。クライアント種別、期限、失効、更新トークンの世代、任意の端末紐づけを管理する。 |
| CSRF token | Cookieを自動送信するブラウザに対する不正な変更要求を防ぐ値。Cookieと要求ヘッダーの一致に加え、Originの完全一致も確認する。 |
