...
- **ワードマーク（アプリ名）**：正式な多色画像ワードマーク「MUDASK」を使い、タグライン（サブタイトル）は付けない。多色はワードマークとキャラクター画像内だけの例外とし、UI部品の配色へ展開しない。
- **キャラクター**：サインインや初回案内に加え、Webでは既存の共通ロゴ右側に限って小さく表示する。契約・支出・見直しの本文や操作部品には置かず、装飾画像として読み上げ対象から外す。
