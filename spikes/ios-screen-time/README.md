...
- [ ] iPhone 実機（iOS 17.4 以降）を USB で Mac に接続できる（`includesPastActivity` が iOS 17.4+ API のため）
...
Step 1〜9 の Xcode GUI 操作を、MacBook 上のコマンド一発に置き換えた自動検証フロー。
`xcodebuild` でビルド → 実機インストール → Appium(XCUITest) でアプリ操作 → Playwright で Web 管理画面を確認する。

> **このフローの対象範囲**：ビルド〜インストール〜起動〜アプリ操作（ID 入力・送信・ステータス確認）〜Web 健全性確認、という**決定的な往復**を自動化する。
> 実利用量の取り込み（閾値到達に 15 分以上かかり非決定的）と、後述の OS シート操作は対象外。

### 構成ファイル

| パス | 役割 |
|---|---|
| `project.yml` | XcodeGen 用のプロジェクト定義（GUI なしで `.xcodeproj`・署名・拡張・ATS を生成）|
| `scripts/setup.sh` | 初回セットアップ自動化（`.env.local`・`Constants.swift` を生成、値を自動検出、`--run` で実行まで）|
| `scripts/refresh-ip.sh` | Mac の LAN IP を再検出し `Constants.swift` の `apiBaseURL` を更新。変化時は再ビルド／ショートカット変更を案内（`run-e2e` が自動で呼ぶ）|
| `scripts/preflight.sh` | ビルド前の前提チェック（ツール・実機・Web 到達・サブスク実在・App Group 整合・**LAN IP 整合**）|
| `scripts/run-e2e-real-device.sh` | 一括実行（生成→ビルド→インストール→Appium→E2E）|
| `e2e/real-device.test.mjs` | Appium(ネイティブ) + Playwright(Web) のオーケストレータ |
| `e2e/config.mjs` | 環境変数の読み取り・検証 |
| `e2e/.env.example` | 環境変数の雛形（`.env.local` にコピーして使用）|

### 手動の事前準備（初回のみ・自動化対象外）

以下は人手でしか実施できない。**最初に 1 回だけ**済ませる。

1. **iPhone 側**
   - **Developer Mode を有効化**：設定 → プライバシーとセキュリティ → デベロッパモード → オン → 再起動。
   - **このコンピュータを信頼**：USB 接続時のダイアログで「信頼」、パスコード入力。
   - **Screen Time 許可と対象アプリ選択**：アプリ上で「Screen Time の許可を求める」→許可、「アプリを選ぶ」で対象を選ぶ。
     これらは OS のプライバシーシートのため XCUITest では安定操作できない。**手動で 1 回**実施しておく
     （E2E は既定 `SKIP_SYSTEM_UI=true` で、未認可なら明示エラーにする）。
...
2. **Mac 側（ツール導入）**
   ...
   # Playwright のブラウザは初回実行時に自動取得（npx playwright install chromium）
   ```

3. **Mac 側（SubBuddy を LAN 公開）**：Step 7 の手順で `apps/web` を `0.0.0.0:3000` で起動し、
   iPhone Safari から `http://<MacのLAN IP>:3000` が開けることを確認しておく。
   > **Mac の LAN IP は再起動・DHCP で変わる。** 変わると iPhone から届かなくなる（`request time out`）。
   > `scripts/run-e2e-real-device.sh` は実行時に `scripts/refresh-ip.sh` を自動で呼び、
   > `Constants.swift` の `apiBaseURL` を現 IP に更新する。**IP が変わった回はアプリの再ビルド・再インストールが要る**
   > ため `SKIP_BUILD=1` を使わないこと。さらに **iPhone のショートカットの送信先 URL も手動で新 IP に直す**
   > （IP は Constants.swift とショートカットの 2 箇所に焼き込まれる）。単体実行は `scripts/refresh-ip.sh`。

4. **`Constants.swift` の用意**：`cp Constants.swift.example Constants.swift` して実値を設定。
   `appGroupID` は環境変数 `IOS_APP_GROUP`（既定 `group.$IOS_BUNDLE_ID`）と**一致**させる。

### 環境変数

| 変数 | 必須 | 説明 |
|---|---|---|
| `IOS_DEVICE_UDID` | ✓ | 実機の UDID（`xcrun devicectl list devices` で確認）|
| `IOS_BUNDLE_ID` | ✓ | 本体アプリの Bundle ID（例 `com.yourname.SubBuddySpike`）|
| `APPLE_TEAM_ID` | ✓ | 署名に使う Apple Developer Team ID（10 桁）|
| `IOS_APP_GROUP` |  | App Group ID（既定 `group.$IOS_BUNDLE_ID`）。Constants.swift と一致必須 |
| `WEB_BASE_URL` |  | Web 管理画面の URL（既定 `http://127.0.0.1:3000`）|
| `IOS_TEST_SUBSCRIPTION_ID` |  | Web 確認の対象サブスク ID（アプリの送信先 DB に実在する ID）|
| `SKIP_SYSTEM_UI` |  | 既定 `true`（OS シートを自動操作しない）|

### Mac 側チェックリスト

**初回のみ（人手）**
1. iPhone：Developer Mode 有効化／USB 接続で「信頼」／Screen Time 許可・対象アプリ選択を 1 回実施（事前準備 1）。
2. Mac：`brew install xcodegen` / `npm i -g appium` / `appium driver install xcuitest`（事前準備 2）。
3. `scripts/setup.sh` を実行：`Constants.swift` と `e2e/.env.local` を自動生成。UDID・LAN IP・**Team ID（署名証明書から）**・**サブスク ID（API から）**・トークンを**すべて自動取得**し、Bundle ID は既定値（手入力なし）。検出できない値だけ警告を出し、環境変数で上書き可。`scripts/setup.sh --run` なら実行まで一気に。

**毎回**
1. iPhone を USB 接続。
2. `apps/web` を LAN 公開で起動（Step 7）。
3. 下記を実行。

### 実行

環境変数は `e2e/.env.local` に書いておけば、毎回これだけ：
...
ビルド前に環境だけを数秒で確認したいときは preflight 単体でも回せる：
...
`.env.local` を使わずインラインで渡すこともできる：

```bash
cd spikes/ios-screen-time
IOS_DEVICE_UDID=00008110-XXXX \
IOS_BUNDLE_ID=com.yourname.SubBuddySpike \
APPLE_TEAM_ID=ABCDE12345 \
IOS_TEST_SUBSCRIPTION_ID=<実在するサブスクID> \
WEB_BASE_URL=http://192.168.1.100:3000 \
scripts/run-e2e-real-device.sh
```

> **Dev Container では実行できない**：`xcodebuild`・実機 USB・Appium(XCUITest) は macOS 専用。
> preflight が非 macOS を検知して即停止する。このリポジトリを Mac のターミナル（コンテナ外）で開いて実行すること。

### 失敗時のアーティファクト

`e2e/artifacts/<日時>/` に保存される。

- `appium.log` … Appium サーバログ
- `xcodebuild.log` / `install.log` … ビルド・インストールログ
- `appium-failure.png` … 失敗時点のネイティブ画面
- `playwright-trace.zip` … Web 操作の trace。閲覧：
  ```bash
...
| 症状 | 対処 |
|---|---|
| `IOS_DEVICE_UDID が未設定です` 等で即終了 | 必須の 3 変数を指定する |
| `'xcodegen' が見つかりません` | `brew install xcodegen` |
| `'appium' が見つかりません` | `npm i -g appium && appium driver install xcuitest` |
| 未認可エラーで停止 | 事前準備 1 の Screen Time 許可・アプリ選択を手動実施 |
| WebDriverAgent の署名エラー | `APPLE_TEAM_ID` が正しいか、Xcode で一度 WDA を実機にビルドして信頼済みか確認 |
| CodeSign で `resource fork, Finder information, or similar detritus not allowed` | iCloud 同期フォルダ（Desktop/Documents 等）が付ける拡張属性が原因。`xattr -cr SubBuddySpike MonitorExtension SubBuddySpike.xcodeproj` で除去（スクリプトは自動実行）。DerivedData は同期外に置かれる |
| Web 確認で `契約情報` が出ない | `IOS_TEST_SUBSCRIPTION_ID` が送信先 DB に実在するか、`WEB_BASE_URL` が届くか確認 |
